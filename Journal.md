...
Phase 4- 
If a folder had a reference to itself or one of the folders above it, it could cause an infinite loop. The recursive method would keep calling itself because it would never reach the end. Eventually, it would run out of space and cause a StackOverflowError.
The iterative method would keep adding the same folders to the stack. The stack would keep growing until the program ran out of memory. This relates to the Smaller-Caller idea because each recursive call should be working on a smaller part of the problem. With a circular reference, the problem does not get smaller, so the method never reaches the stopping point.
